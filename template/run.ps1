...
$config = $global:ApEngineBuildConfig
if (-not $config -or -not $config.APP_NAME) {
    Write-Host "Usage: run.ps1 --target <app_name> [debug|release|static|dynamic|web|msvc|profile|bsthread]"
    exit 1
}


if (-not $config -or -not $config.APP_NAME) {
    Write-Host "Usage: run.ps1 --target <app_name> ..."
    exit 1
}
